...
//this just used for the simple ray caster that intersnzzzzects
...
//ray marching
float dt = .01;
float maxsteps = 10000;
float threshold_ray_casting = 0.000001;
int threshold_steps = -1;
// float min_val = 1.;
...
int change_threshold_steps(int adder){
    threshold_steps = threshold_steps+adder;
...
void vrc_accumulate(const RAY *ray,float t0, float t1, float *out)
{
    ...
    for(int i = 0; i < maxsteps && t0<=t1; i++)
    {
        ...
        // pow(pt.x,2.)+pow(pt.y,2.)+pow(pt.z,2.);
 ...
 *
 */ 
void vrc_accumulate_levoy(const RAY *ray,float t0, float t1, float *out)
{

    
    float prod_inv_alpha = 1.;
    float prod_thresh = 1.;
    //reuse variables
    float threshold_stop = 0.;
    float alpha = 0.;
    float inv_alpha = 1.;
    glm::vec4 actual_col = glm::vec4(0.,0.,0.,0.);
    int i = 0;
    
    int steps_done = 0;
    for(; i < maxsteps && t0<=t1; i++)
    {
        //get point along ray
        glm::vec3 pt = ray->orig+ray->dir*t0;
        //get location of point in volume
         ...
         ***/
         ...
         if(val > ISO_THRESHOLD)
         {  
            //check threshold_steps
            if (threshold_steps>0 && steps_done>=threshold_steps){
                break;
            }

            glm::vec4 tcol = vrt_lookup(transfer,val);
            // EPRINT("tcol.x %f, tcol.y %f, tcol.z %f, tcol.w %f pt.x %f pt.y %f pt.z %f val %f\n",tcol.x,tcol.y,tcol.z,tcol.w, pt.x,pt.y,pt.z,val);
            if(rendphong){
                 //gradient is the normal to the iso-surface at this
                 //location, so it makes sense to use for lighting
                 glm::vec3 grad = vrv_gradient(volume, pt);
            ...
            //scale tcolor by alpha
            tcol = tcol * alpha;
            
            //get 1-alpha
            inv_alpha = 1.-alpha;

            //get multiple of first term prod_{i=0}_{k-1} 1-alpha_{n-i}
            threshold_stop = prod_thresh*inv_alpha;

            //get cumulated tcolor value
            actual_col = actual_col+tcol*prod_inv_alpha;

            //update prod_inv_alpha for next iter
            prod_inv_alpha = prod_inv_alpha * inv_alpha; 

            //check threshold. 
            if (threshold_stop<threshold_ray_casting){
            //     // EPRINT("DONE actual_col.x %f, actual_col.y %f, actual_col.z %f, actual_col.w %f threshold_stop %f steps %d\n",actual_col.x,actual_col.y,actual_col.z,actual_col.w, threshold_stop, i);
                break;
            }
...
//#pragma omp parallel for
    for(int y = 0; y < camera->frame_py; y++){
        for(int x = 0; x < camera->frame_px;x++){
            ...
            vrc_accumulate_levoy(&r,t0,t1,out_rgba+index);
        }
    }
