...
VRVOL* readvol_ui8(const char *fn,size_t gridx,size_t gridy,size_t gridz, glm::vec3 voldim, VINTERP_T interpolation = VI_NEAREST);
VRVOL* readvol_ui8_sphere(size_t gridx,size_t gridy,size_t gridz, glm::vec3 voldim, VINTERP_T interpolation = VI_NEAREST);
...
glm::vec3 get_grads_at_index(const VRVOL* vol,int x, int y, int z);
void get_bicubic_coeff(float coeff[4][4][4],float corner_data[2][2][2][4]);
glm::vec3 gradient_tricubic_ui8(const VRVOL*vol, glm::vec3 pt);
float get_bicubic_coeff_at_index(const VRVOL *vol, int x, int y, int z, int i, int j, int k);
void vrv_initcoeff(VRVOL *vol);
