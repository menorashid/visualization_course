...
cam_init(CAMERA *cam, int pixels_x, int pixels_y)
{
    ...
    // glm::vec3 end = glm::normalize(glm::vec3( -0.932080, 0.309109, 0.188889));

    // start = glm::normalize(start);
    // end = glm::normalize(end);

    //get angle and axis
    // float rad = acosf(glm::dot(cam->eye,end));
    // glm::vec3 axis = glm::normalize(glm::cross(cam->eye,end));

    // glm::quat rot = glm::angleAxis(rad,axis);
    // glm::quat rot_c = glm::conjugate(rot);
    // cam->eye = glm::normalize(rot*cam->eye*rot_c);
    // cam->up = glm::normalize(rot*cam->up*rot_c);
    ...
    cam->eye = glm::normalize(glm::vec3( -0.932080, 0.309109, 0.188889));
    cam->dir = glm::normalize(-1.f * cam->eye);
    // glm::normalize(glm::vec3( 0.932080, -0.309109, -0.188889));
    cam->up = glm::normalize(glm::vec3( -0.254760, -0.022317, 0.966747));
