...
def generate_samples(n_pts):
    ...
    assert len(np.unique(pts[:,0]))==pts.shape[0]
    assert len(np.unique(pts[:,1]))==pts.shape[0]
    # pts = pts * 2 -1
    return pts
...
def serrated_sphere(pts):
    pts = pts*8
    f_val = np.sqrt(np.sum(pts**2,1))
    f_val = np.sin(f_val)
    min_val = -1
    max_val = 1
    return f_val,min_val,max_val
...
def get_scattered_pts(scatter_fun,n_pts):
    ...
    elif scatter_fun[0]=='serrated_sphere':
        f_vals, min_val, max_val = serrated_sphere(pts)
